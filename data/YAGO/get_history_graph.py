...
def update_history_cache(ee, s_his, s_his_t, o_his, o_his_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t, num_e):
    if len(s_his_cache[ee]) != 0:
        s_his[ee].append(s_his_cache[ee].copy())
        s_his_t[ee].append(s_his_cache_t[ee])
        s_his_cache[ee], s_his_cache_t[ee] = [], None
    if len(o_his_cache[ee]) != 0:
        o_his[ee].append(o_his_cache[ee].copy())
        o_his_t[ee].append(o_his_cache_t[ee])
        o_his_cache[ee], o_his_cache_t[ee] = [], None


def process_quadruple(idx, quadruple, s_his, s_his_t, o_his, o_his_t, s_history_data, o_history_data, s_history_data_t, o_history_data_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t):
    s, r, o, t = quadruple[0], quadruple[1], quadruple[2], quadruple[3]
    s_history_data[idx], o_history_data[idx] = s_his[s].copy(), o_his[o].copy()
    s_history_data_t[idx], o_history_data_t[idx] = s_his_t[s].copy(), o_his_t[o].copy()
...
def save_dataset(prefix, s_history_data, s_history_data_t, o_history_data, o_history_data_t,
                 s_label, o_label, s_related, o_related):
    with open(f'{prefix}_history_sub.txt', 'wb') as fp:
        pickle.dump([s_history_data, s_history_data_t], fp)
    with open(f'{prefix}_history_ob.txt', 'wb') as fp:
        pickle.dump([o_history_data, o_history_data_t], fp)
    with open(f'{prefix}_s_label.txt', 'wb') as fp:
        pickle.dump(s_label, fp)
    with open(f'{prefix}_o_label.txt', 'wb') as fp:
        pickle.dump(o_label, fp)
    with open(f'{prefix}_s_frequency.txt', 'wb') as fp:
        pickle.dump(s_related, fp)
    with open(f'{prefix}_o_frequency.txt', 'wb') as fp:
        pickle.dump(o_related, fp)
...
if __name__ == '__main__':
    ...
    for tim in train_times:
        graph_dict_train[tim] = get_big_graph(get_data_with_t(train_data, tim), num_r)
    for i, train in enumerate(train_data):
        t = train[3]
        if latest_t != t:
            for ee in range(num_e):
                update_history_cache(ee, s_his, s_his_t, o_his, o_his_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t, num_e)
            latest_t = t
        process_quadruple(i, train, s_his, s_his_t, o_his, o_his_t, s_history_data, o_history_data, s_history_data_t, o_history_data_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t)

    s_label_train, o_label_train, s_history_related_train, o_history_related_train = get_history_target(train_data, s_history_data, o_history_data)
    with open('train_graphs.txt', 'wb') as fp:
        pickle.dump(graph_dict_train, fp)
    save_dataset('train', s_history_data, s_history_data_t, o_history_data, o_history_data_t, s_label_train, o_label_train, s_history_related_train, o_history_related_train)
    ...
    for i, dev in enumerate(dev_data):
        if latest_t != dev[3]:
            for ee in range(num_e):
                update_history_cache(ee, s_his, s_his_t, o_his, o_his_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t, num_e)
            latest_t = dev[3]
        process_quadruple(i, dev, s_his, s_his_t, o_his, o_his_t, s_history_data_dev, o_history_data_dev, s_history_data_dev_t, o_history_data_dev_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t)
    s_label_dev, o_label_dev, s_history_related_dev, o_history_related_dev = get_history_target(dev_data, s_history_data_dev, o_history_data_dev)
    save_dataset('dev', s_history_data_dev, s_history_data_dev_t, o_history_data_dev, o_history_data_dev_t, s_label_dev, o_label_dev, s_history_related_dev, o_history_related_dev)
    ...
    for i, test in enumerate(test_data):
        if latest_t != test[3]:
            for ee in range(num_e):
                update_history_cache(ee, s_his, s_his_t, o_his, o_his_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t, num_e)
            latest_t = test[3]
        process_quadruple(i, test, s_his, s_his_t, o_his, o_his_t, s_history_data_test, o_history_data_test, s_history_data_test_t, o_history_data_test_t, s_his_cache, s_his_cache_t, o_his_cache, o_his_cache_t)
    s_label_test, o_label_test, s_history_related_test, o_history_related_test = get_history_target(test_data, s_history_data_test, o_history_data_test)
    save_dataset('test', s_history_data_test, s_history_data_test_t, o_history_data_test, o_history_data_test_t, s_label_test, o_label_test, s_history_related_test, o_history_related_test)
