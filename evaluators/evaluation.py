import torch
from tqdm import tqdm
from .utils import _move_batch_to_device

def execute_test(batch_size, total_data_tensor, model, test_loader, device=None):
    s_ranks1, o_ranks1, all_ranks1 = [], [], []
    s_ranks2, o_ranks2, all_ranks2 = [], [], []
    s_ranks3, o_ranks3, all_ranks3 = [], [], []
    total_loss = 0.0
    total_loss2 = 0.0
    total_loss3 = 0.0
    batch_count = 0

    if device is None:
        device = total_data_tensor.device

    model.eval()
    with torch.no_grad():
        pbar = tqdm(test_loader, desc="Testing", unit='batch')
        for batch_data in pbar:
            batch_data = _move_batch_to_device(batch_data, device)

            cur_loss, sub_rank1, obj_rank1, \
            sub_rank2, obj_rank2, \
            sub_rank3, obj_rank3, ce_all_acc = model(batch_data, 'Test', total_data_tensor)

            s_ranks1 += sub_rank1
            o_ranks1 += obj_rank1
            all_ranks1 += sub_rank1 + obj_rank1

            s_ranks2 += sub_rank2
            o_ranks2 += obj_rank2
            all_ranks2 += sub_rank2 + obj_rank2

            s_ranks3 += sub_rank3
            o_ranks3 += obj_rank3
            all_ranks3 += sub_rank3 + obj_rank3

            total_loss += cur_loss.item() if hasattr(cur_loss, 'item') else float(cur_loss)
            batch_count += 1

    model.train()

    avg_loss = total_loss / batch_count if batch_count > 0 else 0.0

    return s_ranks1, o_ranks1, all_ranks1, \
           s_ranks2, o_ranks2, all_ranks2, \
           s_ranks3, o_ranks3, all_ranks3, \
           avg_loss